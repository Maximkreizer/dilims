  ...
  <v-card border class="d-flex flex-column" :style="{ height: tableHeight + 'px' }" style="overflow: hidden;">
    ...
    >
      ...
      <template v-slot:item.status="{ item }">
        <div @click.stop="startEdit(item, 'status')" class="editable-cell">
      ...
      <template v-slot:item.technicalAssistantId="{ item }">
        <div @click.stop="startEdit(item, 'technicalAssistantId')" class="editable-cell">
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          Möchten Sie das Projekt <strong>{{ itemToDelete?.projectNumber }}</strong> wirklich löschen? 
...
const emit = defineEmits<{
  ...
  (e: 'delete', project: Project): void; // Falls der Parent das Handling übernehmen soll
}>();
...
async function executeDelete() {
  if (itemToDelete.value) {
    // Hier können wir entweder direkt löschen oder ein Event senden
    // Da die Tabelle "dumm" sein sollte, senden wir das Event an den Parent
...
async function quickSave(item: Project) { const saved = await api.saveProject(item); emit('project-updated', saved); }
...
function startHeightResize(event: MouseEvent) {
  ...
  const onMouseMove = (e: MouseEvent) => { 
    ...
    tableHeight.value = Math.max(150, startHeight + (e.pageY - startY)); 
  };
  const onMouseUp = () => { document.removeEventListener('mousemove', onMouseMove); document.removeEventListener('mouseup', onMouseUp); document.body.style.cursor = ''; };
